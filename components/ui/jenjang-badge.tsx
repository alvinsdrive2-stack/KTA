...
export function getJenjangCategory(jenjang: number | string): 'OPERATOR' | 'TEKNISI' | 'AHLI' {
  const jenjangNum = typeof jenjang === 'string' ? parseInt(jenjang, 10) : jenjang
  if (jenjangNum <= 3) return 'OPERATOR'
  if (jenjangNum <= 6) return 'TEKNISI'
  return 'AHLI'
}

export function getJenjangColor(category: 'OPERATOR' | 'TEKNISI' | 'AHLI'): string {
  switch (category) {
    case 'OPERATOR':
