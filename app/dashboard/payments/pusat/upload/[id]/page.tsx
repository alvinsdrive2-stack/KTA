...
export default function PusatUploadProofPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmitUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        router.push('/dashboard/payments/pusat?upload_success=true')
      } else {
    ...
    return (
      <div className="space-y-4">
        ...
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            Invoice ini sudah dalam status {payment.status}
          </AlertDescription>
        </Alert>
      </div>
    )
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Upload Bukti Pembayaran (Pusat)</h1>
